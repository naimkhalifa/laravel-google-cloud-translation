...
describe('GoogleCloudTranslationCommand', function () {
    afterEach(function () {
    ...
    it('exits if --source option is not provided', function () {
        $this->artisan('google-cloud-translation:translate-strings --target=fr --file=tests/static/en/translations.php')
            ->assertExitCode(1)
            ->expectsOutput('The "--source" option must be set.');
    });

    it('exits if --target option is not provided', function () {
        $this->artisan('google-cloud-translation:translate-strings --source=en --file=tests/static/en/translations.php')
            ->assertExitCode(1)
            ->expectsOutput('The "--target" option must be set.');
    });

    it('exits if --file option is not provided', function () {
        $this->artisan('google-cloud-translation:translate-strings --target=fr --source=en')
            ->assertExitCode(1)
    ...
    it('exits if the file is not valid', function () {
        ...
        $this->artisan('google-cloud-translation:translate-strings --source=en --target=fr --file='.$invalidFile)
            ->assertExitCode(1)
            ->expectsOutput("File $invalidFile does not exist.");
    });

    it('exits if provided source locale is not supported', function () {
        $this->artisan('google-cloud-translation:translate-strings --source=xx --target=fr --file=tests/static/en/translations.php')
            ->assertExitCode(1)
            ->expectsOutput('Invalid source language.');
    });

    it('exits if provided target locale is not supported', function () {
        $this->artisan('google-cloud-translation:translate-strings --source=en --target=xx --file=tests/static/en/translations.php')
            ->assertExitCode(1)
    ...
    it('outputs a file containing all translations for provided language without confirming if --dryrun is set to false', function () {
        $this->artisan('google-cloud-translation:translate-strings --source=en --target=fr --file=tests/static/en/translations.php --dryrun=false')
            ->assertExitCode(0);
    ...
    it('outputs the content to the terminal if --dryrun is set to true', function () {
        $this->artisan('google-cloud-translation:translate-strings --source=en --target=fr --file=tests/static/en/translations.php --dryrun=true')
    ...
    it('asks for confirmation and write the translations to the file if --dryrun is set to true and the user confirms', function () {
        ...
        $this->artisan('google-cloud-translation:translate-strings --source=en --target=fr --file='.$filePath)
